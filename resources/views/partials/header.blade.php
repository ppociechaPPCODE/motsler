...
<header class="sticky top-0 z-40 w-full border-b border-zinc-200 bg-white shadow-[0_1px_0_0_rgba(15,23,42,.06)]">
    <div class="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav class="hidden items-baseline gap-6 lg:flex" aria-label="{{ __('aria.menu') }}">
            ...
            <a class="text-base font-semibold text-[#003174] hover:text-[#001348]" href="https://motsler24.pl/" target="_blank" rel="noopener noreferrer">{{ __('nav.shop') }}</a>
            <a class="text-base font-semibold text-[#003174] hover:text-[#001348]" href="{{ locale_route('contact', ['locale' => $l]) }}">{{ __('nav.contact') }}</a>
        </nav>
        <div class="flex items-center gap-3">
            <a class="hidden rounded-full bg-[#ffad03] px-4 py-2 text-sm font-semibold text-[#001348] shadow-sm transition hover:bg-[#ffc94d] sm:inline-flex" href="{{ locale_route('contact', ['locale' => $l]) }}">{{ __('nav.consultation') }}</a>
            ...
            <details class="relative lg:hidden">
                ...
                <div class="absolute right-0 mt-2 w-56 rounded-lg border border-zinc-200 bg-white p-2 shadow-lg">
                    <a class="mb-2 block rounded-full bg-[#ffad03] px-3 py-2 text-center text-sm font-semibold text-[#001348] transition hover:bg-[#ffc94d]" href="{{ locale_route('contact', ['locale' => $l]) }}">{{ __('nav.consultation') }}</a>
